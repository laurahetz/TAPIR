...
// Code adapted from https://github.com/yacovm/PoL
...
func PowerSeries(n int, exp *Zr) Vec {
...
func BitsBigEndian(n *Zr, bitLen int) []uint8 {
	return ReverseBits(Bits(n, bitLen))
}

func Bits(n *Zr, bitLen int) []uint8 {
...
func RandGenVec(n int, context string) []*G1 {
	v := make([]*G1, n)
...
func (v Vec) InnerProd(v2 Vec) *Zr {
	if len(v) != len(v2) {
...
func (g1v G1v) InnerProd(g2v G2v) *Gt {
	if len(g1v) != len(g2v) {
...
func HashToG1(in []byte) *G1 {
	return c.HashToG1(in)
}

func FieldElementFromBytes(digest []byte) *Zr {
	// ensure input to feFrom256Bits is 32 bytes
	d := make([]byte, 32)
	copy(d[:len(digest)], digest)
