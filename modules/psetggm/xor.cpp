...
extern "C"
{
    ...
    void xor_into(uint8_t* out, uint8_t* in, unsigned int elem_size) {
        if ((elem_size % 32) == 0)
        {
            ...
            fprintf(stderr, "xor_into: elem_size not a multiple of 16 or 32\n");
            exit(1);
        }
        return;
    }
    ...
    void xor_all_rows(uint8_t* db, unsigned int num_elems,
        ...
        if ((elem_size % 32) == 0)
        {
            ...
            fprintf(stderr, "xor_all_rows: elem_size not a multiple of 16 or 32\n");
            exit(1);
        }
        return;
    }
    ...
    void xor_single_pass(uint8_t* db, uint32_t* inverse_perm, unsigned int perm_size,  uint8_t* parities, unsigned int elem_size) {
        ...
        if ((elem_size % 32) == 0) {
            for (int i = 0; i < perm_size; i++)
            {
                ...
                for (int b = 0; b < (elem_size / 32); b++)
                {
                    __m256i par256 = _mm256_loadu_si256((__m256i *)curr_parity + b);
                    __m256i elem = _mm256_loadu_si256(block + b);
            ...
            for (int i = 0; i < perm_size; i++)
            {
                ...
                for (int b = 0; b < (elem_size / 16); b++)
                {
                    __m128i par128 = _mm_loadu_si128((__m128i *)curr_parity + b);
                    __m128i elem = _mm_loadu_si128(block + b);
            ...
            fprintf(stderr, "xor_single_pass: elem_size not a multiple of 32\n");
            exit(1);
        }
        return;
    }
    ...
    void xor_into(uint8_t* out, uint8_t* in, unsigned int elem_size) {
         __m128i *block = (__m128i *)(in);
        // if ((elem_size % 16) != 0) {
        //     // todo add logic to deal with non-multiples of 16 bytes         
        //     fprintf(stderr, "xor_into: elem_size not a multiple of 16\n");
        //     exit(1);
        // }
        for (int b = 0; b < (elem_size / 16); b++)
        {
    ...
    void xor_single_pass(uint8_t* db, uint32_t* inverse_perm, unsigned int perm_size,  uint8_t* parities, unsigned int elem_size) {
        ...
        if ((elem_size % 16) != 0) {
            // todo add logic to deal with non-multiples of 16 bytes         
            fprintf(stderr, "xor_single_pass: block_len not a multiple of 16\n");
            exit(1);
        }
        uint8_t* curr_db = db;
        for (int i = 0; i < perm_size; i++)
